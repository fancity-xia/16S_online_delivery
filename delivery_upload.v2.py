...
class Online_upload():
	...
	'''
	def __init__(self, analysisid, analysis_path="Null", bo = "/root/Software/miniconda3/envs/qiime1/bin/bo"):
	...
	def send_email(self, other=""):
		...
		eestring = eestring.format(self.analysisid, ','.join(self.baseinfo.myconfig['project_num']), ','.join(self.baseinfo.myconfig['projects']), self.usr, self.password)
...
if __name__  == '__main__':
	...
	p = Online_upload(analysisid, analysis_path)
	#p.upload_main()
	p.send_email()
	p.update_mysql()
